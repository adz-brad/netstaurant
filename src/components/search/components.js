...
const SearchHit = ({ hit: {objectID, title, handle, tags, sku, productType, vendor, images } }) => (

    <article key={objectID} className="flex flex-row">

    {images.slice(0, 1).map((image) => {
        return(<GatsbyImage image={image} className="w-2/5 m-auto"/>
    )})}
    ...
    <div className="flex flex-col w-3/5">
        ...
        <Button
            className="mx-auto mb-2 mt-auto px-2 py-1 text-white text-lg md:text-xl md:px-3 md:py-2"
            text="View Product"
            url={`/products/${handle}`}
        />
